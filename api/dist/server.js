...
export async function buildServer() {
    ...
    router.get('/jobs', async (req, res, next) => {
        try {
            ...
            const minSalary = req.query.minSalary ? Number.parseInt(req.query.minSalary, 10) : undefined;
            const filters = {
                limit,
    ...
    router.post('/resume/analyze', RESUME_RATE_LIMITER, upload.single('file'), async (req, res, next) => {
        try {
            if (!req.file) {
    ...
    router.post('/resume/llm_analyze', RESUME_RATE_LIMITER, upload.single('file'), async (req, res, next) => {
        try {
            if (!req.file) {
                res.status(400).json({ error: 'missing_file', message: 'Resume file is required.' });
                return;
