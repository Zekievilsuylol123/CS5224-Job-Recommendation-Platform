...
function inferEducationLevel(education) {
    ...
    if (degrees.some(d => d.includes('phd') || d.includes('doctor')))
        return 'PhD';
    if (degrees.some(d => d.includes('master') || d.includes('msc') || d.includes('mba')))
        return 'Masters';
    if (degrees.some(d => d.includes('bachelor') || d.includes('bsc') || d.includes('ba ') || d.includes('b.a')))
        return 'Bachelors';
    if (degrees.some(d => d.includes('diploma')))
        return 'Diploma';
 ...
 */
function inferYearsExperience(experience) {
...
function corsValidator(origin, callback) {
    if (!origin)
...
function parseProfileFromRequest(req) {
...
function inferPlanFromProfile(profile) {
...
export async function buildServer() {
    ...
    router.get('/profile', requireAuth, async (req, res, next) => {
        try {
            ...
            const profile = data ? {
                ...
                plan: data.plan,
                latestCompassScore: data.latest_compass_score ? (typeof data.latest_compass_score === 'object'
    ...
    router.get('/jobs', optionalAuth, async (req, res, next) => {
        try {
            ...
            const pageSize = req.query.pageSize ? Number.parseInt(req.query.pageSize, 10) : 50;
    ...
    router.get('/jobs/:id/assessment', requireAuth, async (req, res, next) => {
        try {
            ...
            res.json({
                ...existingAssessment,
                subscores: existingAssessment.subscores,
                evidence: existingAssessment.evidence,
                gaps: existingAssessment.gaps,
                questions_for_interview: existingAssessment.questions_for_interview,
                recommendations_to_candidate: existingAssessment.recommendations_to_candidate,
                compass_score: existingAssessment.compass_score,
                from_cache: true
            });
        }
        catch (error) {
    ...
    router.post('/jobs/:id/analyze', requireAuth, async (req, res, next) => {
        try {
            ...
            if (!regenerate) {
                ...
                if (existingAssessment) {
                    ...
                    res.json({
                        ...
                        questions_for_interview: existingAssessment.questions_for_interview,
                        recommendations_to_candidate: existingAssessment.recommendations_to_candidate,
                        compass_score: existingAssessment.compass_score, // Include stored COMPASS score
                        from_cache: true
                    });
                    return;
                }
            ...
            let user_prompt = fs.readFileSync(path.join("resources", "llm_prompts", "profile_jd_score_user.txt"), "utf8");
    ...
    router.post('/assessments/compass', requireAuth, async (req, res, next) => {
        try {
            ...
            await supabaseAdmin.from('compass_scores').insert({
                ...
                total_score: score.totalRaw ?? score.total,
            ...
            await supabaseAdmin
                .from('profiles')
                .update({
                latest_compass_score: score.totalRaw ?? score.total,
    ...
    router.patch('/applications/:id', requireAuth, async (req, res, next) => {
        try {
            ...
            if (req.body.appliedAt)
                updateData.applied_at = new Date(req.body.appliedAt).toISOString();
            const { data, error } = await supabaseAdmin
                .from('applications')
                .update(updateData)
                .eq('id', req.params.id)
    ...
    router.post('/resume/analyze', requireAuth, RESUME_RATE_LIMITER, upload.single('file'), async (req, res, next) => {
        try {
            if (!req.file) {
