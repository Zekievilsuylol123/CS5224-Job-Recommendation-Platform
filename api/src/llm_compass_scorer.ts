...
export async function scoreCompassWithLLM(input: AssessmentInput): Promise<CompassScore> {
  ...
  const systemPrompt = fs.readFileSync(
    path.join(__dirname, "..", "..", "resources", "llm_prompts", "compass_scoring.txt"), 
    "utf8"
  );
  ...
  const response = await client.responses.create({
    ...
    text: {
      format: zodTextFormat(CompassScoreSchema as any, "compass_score"),
    },
  });
 ...
 */
function formatAssessmentInput(input: AssessmentInput): string {
  ...
  if (user.nationality) prompt += `**Nationality**: ${user.nationality}\n`;
  ...
  if (job) {
    prompt += "\n# JOB DETAILS\n\n";
    if (job.title) prompt += `**Job Title**: ${job.title}\n`;
    if (job.company) prompt += `**Company**: ${job.company}\n`;
    if (job.industry) prompt += `**Industry**: ${job.industry}\n`;
