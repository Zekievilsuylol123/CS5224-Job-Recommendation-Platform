...
import type { WebsiteData, ParsedKnowledgeData } from './types.js';
...
async function summarizeWebContent(markdown: string, url: string): Promise<string> {
  try {
...
export function normalizeWebsiteToKnowledge(data: WebsiteData): ParsedKnowledgeData {
...
export async function scrapeAndParseWebsite(url: string): Promise<{
  raw: WebsiteData;
