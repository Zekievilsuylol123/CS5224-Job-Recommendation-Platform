...
const COMPANY_WEBHOOKS: Record<string, string> = {
  'grab': 'grab-jd',
...
const JD_CACHE = new Map<string, { data: JDWebhookResponse; timestamp: number }>();
const CACHE_TTL = 15 * 60 * 1000; // 15 minutes

/**
 * Get the webhook name for a company
 */
function getWebhookName(company: string): string {
 ...
 */
export async function fetchJobDescription(jobUrl: string, company: string): Promise<JDWebhookResponse | null> {
 ...
 */
export function isInternSGCompany(company: string): boolean {
  const normalizedCompany = company.toLowerCase().trim();
  return !Object.keys(COMPANY_WEBHOOKS).some(key => 
    normalizedCompany === key || normalizedCompany.includes(key)
  );
}
